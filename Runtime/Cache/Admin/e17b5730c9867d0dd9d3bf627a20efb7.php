...
<div class="shenhe col-sm-9" style="margin-top:20px;">
    ...
    <form action="<?php echo U('Index/shenhe');?>" class="form-horizontal" method="post">
      <div class="form-group">
        <label for="name" class="col-sm-2  control-label">选择查询的信息 :</label>
		<div class="col-sm-2 input-parent">
          <input type="text" name="year" placeholder="2017" class="form-control vinput" id="year" value="<?=$year?>"/>
        </div>
		<div class="col-sm-2 input-parent">
    ...
    <table class="table" style="margin-top:35px;">
     ...
     <?php if(is_array($users)): foreach($users as $key=>$u): ?><tr class="primary">
        <td>1</td>
		<td><?php echo ($u["account"]); ?></td>
		<td><?php echo ($u["real_account"]); ?></td>
        <td>
			<form action="<?php echo U('Index/checkuser');?>" method="post">
			<input type="hidden" name="account" value="<?php echo ($u["account"]); ?>"/>
			<button class="btn btn-primary shehe" type="submit" >审核</button>
			</form>
		</td>
        <td name="<?php echo ($u["pass"]); ?>"><?php echo ($u["pass"]); ?></td>
        <td><?php echo ($u["reason"]); ?></td>
		<form action="<?php echo U('Index/delet');?>" method="post">
		<input type="hidden" name="account" value="<?php echo ($u["account"]); ?>"/>
        <td><button class="btn btn-primary" type="submit" >删除</button></td>
		</form>
      </tr><?php endforeach; endif; ?>
    </table>
    <form action="<?php echo U('Index/adduser');?>" class="form-horizontal" style="margin-bottom:150px;">
      <div class="form-group" style="float:right;margin-top:20px;">
        <div class="col-sm-12 input-parent">
